...
export function buildHistory(
  ...
  for (let i = 0; i < historyMessages.length; i++) {
    ...
    // systemPrompt / tools were already extracted by streamKiro via
    // getCurrentSystemPrompt/getCurrentTools. Skipping here prevents a
    // SystemMessage from falling into the toolResult branch below (which
    // would emit toolUseId=undefined orphan carriers) and avoids sending
    // system content twice on the wire.
